...
export const WithControls: Story = {
  render: (args: PaginationProps) => {
    const [idx, setIdx] = useState(Math.min(args.current ?? 0, Math.max(0, (args.total ?? 1) - 1)))
    ...
    return (
      <View style={{ gap: 16, alignItems: 'center' }}>
        ...
        <View style={{ flexDirection: 'row', gap: 12 }}>
          <Pressable onPress={prev} style={btnStyle}><Text style={btnText}>이전</Text></Pressable>
          <Pressable onPress={next} style={btnStyle}><Text style={btnText}>다음</Text></Pressable>
        </View>
